(() => {
  ...
  async function fetchYoutubeiTranscript(videoId) {
    ...
    return { segments, trackLabel: 'YouTube transcript endpoint' };
  }
      ...
      for (const track of orderedTracks) {
        try {
          ...
          if (segments.length) {
            return {
              segments,
              trackLabel: textFromName(track.name) || `${source.label} ${track.languageCode || videoId || ''}`.trim()
            };
          }
        } catch (error) {
  ...
  async function loadTranscriptForCurrentVideo(videoId) {
    ...
    try {
      ...
      if (opened) {
        ...
        if (panelRows.length) {
          return { segments: panelRows, trackLabel: 'Transcript panel' };
        }
  ...
  async function refreshTranscript(force = false) {
    ...
    try {
      const { segments, trackLabel } = await loadTranscriptForCurrentVideo(videoId);
      ...
      setStatusParts({
        transcriptText: `字幕を読み込みました。${state.rawSegments.length} 行を ${state.groupedSegments.length} ブロックに連結しました。`,
        transcriptError: false
      });
      setupTranslationState();
      syncSubtitle();
